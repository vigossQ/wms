    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("input[tag=salePrice],input[tag=number]").blur(function () {
                var salePrice = parseFloat($(this).closest("tr").find("input[tag=salePrice]").val()) || 0;
                var number = parseFloat($(this).closest("tr").find("input[tag=number]").val()) || 0;
                $(this).closest("tr").find("span[tag=amount]").text((salePrice * number).toFixed(2));
            });
            //添加明细
                        ...
                        <table class="edit_table" cellspacing="0" cellpadding="0" bstockIncome="0" style="width: auto">
                            ...
                            <c:if test="${stockOutcomeBill.id!=null}">
                                <c:forEach items="${stockOutcomeBill.items}" var="item">
                                    <tr>
                                        ...
                                        <td><input tag="salePrice" name="stockOutcomeBill.items.salePrice"
                                                   class="ui_input_txt00" value="${item.salePrice}"/></td>
                                        <td><input tag="number" name="stockOutcomeBill.items.number"
                                                   class="ui_input_txt00" value="${item.number}"/></td>
                                        <td><span tag="amount">${item.amount}</span></td>
                                        <td><input tag="remark" name="stockOutcomeBill.items.remark"
                            ...
                            <c:if test="${stockOutcomeBill.id==null}">
                                <tr>
                                    ...
                                    <td><input tag="salePrice" name="items.salePrice"
