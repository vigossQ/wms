        ...
        <div class="ui_content">
            <table cellspacing="0" cellpadding="0" width="100%" align="left" bstockIncome="0">
                <tr>
                    ...
                    <td class="ui_text_lt">
                        <input name="sn" class="ui_input_txt02" value="${stockIncomeBill.sn}" disabled="disabled"/>
                        <%--&nbsp;<span name="sn">${stockIncomeBill.sn}</span>--%>
                    </td>
                </tr>
                <tr>
                    <td class="ui_text_rt" width="140">仓库:</td>
                    <td class="ui_text_lt">
                        <input name="sn" class="ui_input_txt02" value="${stockIncomeBill.depot.name}" disabled="disabled"/>
                        <%--&nbsp;<span name="sn">${stockIncomeBill.depot.name}</span>--%>
                    </td>
                </tr>
                <tr>
                            ...
                            <c:forEach items="${stockIncomeBill.items}" var="item">
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="stockIncomeBill.items.product.id" tag="pid" value="${item.product.id}"/>
                                    </td>
                                    <td><span tag="brand">${item.product.brandName}</span></td>
                                    <td><input tag="costPrice" name="stockIncomeBill.items.costPrice" disabled="disabled"
                                               class="ui_input_txt00" value="${item.costPrice}"/></td>
                                    <td><input tag="number" name="stockIncomeBill.items.number" disabled="disabled"
                                               class="ui_input_txt00" value="${item.number}"/></td>
                                    <td><span tag="amount">${item.amount}</span></td>
                                    <td><input tag="remark" name="stockIncomeBill.items.remark" disabled="disabled"
